...
KNOWN_SOCIAL_TASKS = [
    ...
    "join_partner",
    "boost_channel",
    "join_moola_solana",  # تسک جدید سولانا

    # واتساپ (WhatsApp)
    "follow_whatsapp",   # تسک جدید واتساپ
...
async def process_account(acc):
    ...
    async with aiohttp.ClientSession(headers=headers, timeout=timeout) as session:
        ...
        # ۱. دریافت اولیه پروفایل کاربر (همراه با مکانیزم Retry)
        ...
        # ۵. مرحله نهایی: کلیم پاداش ماین و استارت مجدد چرخه
        print(f"[{acc_name}] Finalizing: Claiming pending balance & starting mine...")
        
        # الف) کلیم امتیازات کسب‌شده دوره
        try:
            async with session.post(f"{BASE_API}/mine/claim", json={}) as c_resp:
                if c_resp.status == 200:
                    ...
                    print(f"[{acc_name}] Mining claim response: {c_resp.status}")
        except Exception as e:
            print(f"[{acc_name}] Error claiming mine: {e}")

        await asyncio.sleep(1.5)

        # ب) فعال‌سازی مجدد تایمر ماینینگ با استارت
        try:
            async with session.post(f"{BASE_API}/mine/start", json={}) as start_resp:
                if start_resp.status == 200:
                    ...
                    print(f"[{acc_name}] [+] Mining Started: Active={mining.get('active')} | EndsAt={mining.get('endsAt')}")
                else:
                    print(f"[{acc_name}] [-] Start mine failed: {start_resp.status}")
        except Exception as e:
